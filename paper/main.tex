...
Experiments on GenImage with ten random seeds show that \ours{} significantly improves JPEG compression robustness in the multi-generator training setting ($88.6{\pm}2.4\%$ \vs{} $82.9{\pm}3.9\%$ at $Q{=}30$; paired $t$-test $p{=}0.001$, $n{=}10$), with lower variance and a particularly strong advantage on unseen generators ($+9.1\%$ on Wukong $Q{=}30$, $p{<}0.001$).
A no-augmentation experiment demonstrates inherent feature-level robustness (AP$=$0.828 \vs{} 0.623 without JPEG training augmentation, $n{=}5$).
...
For instance, a CLIP+LoRA detector trained on clean images drops from 97.8\% to 82.9\% under JPEG quality factor $Q{=}30$ in multi-generator evaluation.
...
\begin{enumerate}
    ...
    \item We demonstrate through systematic evaluation with 10 random seeds that patch-level transitions significantly improve compression robustness under multi-generator training ($+5.7\%$ over CLIP+LoRA at $Q{=}30$, $p{=}0.001$), with lower variance. Results generalize to ViT-L/14.
...
All experiments are repeated with 10 random seeds and reported as mean$\pm$std (sample standard deviation). Statistical significance is assessed via paired $t$-tests.
...
\begin{table}[t]
\centering
\caption{Cross-generator accuracy (\%) under multi-generator training (SD1.4+BigGAN+ADM). Mean$\pm$std ($n{=}10$).}
\label{tab:cross_gen}
...
\midrule
CLIP+LoRA      & 97.8\scriptsize$\pm$0.5 & 93.6\scriptsize$\pm$1.0 & 95.5\scriptsize$\pm$0.7 & 96.6\scriptsize$\pm$1.1 & 95.9\scriptsize$\pm$0.9 & 66.6\scriptsize$\pm$3.2 \\
CLS-LTD        & \textbf{98.6}\scriptsize$\pm$0.2 & \textbf{93.6}\scriptsize$\pm$1.2 & 95.1\scriptsize$\pm$1.1 & \textbf{97.9}\scriptsize$\pm$0.5 & \textbf{96.9}\scriptsize$\pm$0.4 & 69.5\scriptsize$\pm$3.5 \\
\ours{}-mp     & 97.7\scriptsize$\pm$0.3 & 92.2\scriptsize$\pm$1.6 & 94.9\scriptsize$\pm$1.2 & 97.1\scriptsize$\pm$1.5 & 96.1\scriptsize$\pm$0.4 & \textbf{71.2}\scriptsize$\pm$3.7 \\
\textbf{\ours{}} & 97.5\scriptsize$\pm$0.3 & 92.6\scriptsize$\pm$1.3 & \textbf{95.3}\scriptsize$\pm$0.9 & 96.3\scriptsize$\pm$1.0 & 95.7\scriptsize$\pm$0.8 & 70.1\scriptsize$\pm$3.2 \\
\bottomrule
\end{tabular}
\end{table}

\Cref{tab:jpeg} presents the key result: JPEG compression robustness.
\ours{} achieves $88.6{\pm}2.4\%$ at $Q{=}30$, outperforming CLIP+LoRA by $+5.7\%$ ($82.9{\pm}3.9\%$).
A paired $t$-test confirms statistical significance ($p{=}0.001$, $n{=}10$).
The improvement is also significant on unseen generators (\Cref{tab:unseen_jpeg}): Wukong $Q{=}30$ yields $p{<}0.001$.

\begin{table}[t]
\centering
\caption{JPEG robustness (\%) under multi-generator training. Evaluated on the training generator (SD1.4) at various JPEG quality factors. Mean$\pm$std ($n{=}10$).}
\label{tab:jpeg}
...
\midrule
CLIP+LoRA      & 97.8\scriptsize$\pm$0.5 & 97.5\scriptsize$\pm$0.6 & 94.7\scriptsize$\pm$1.8 & 89.0\scriptsize$\pm$3.3 & 82.9\scriptsize$\pm$3.9 \\
CLS-LTD        & \textbf{98.6}\scriptsize$\pm$0.2 & \textbf{98.3}\scriptsize$\pm$0.2 & 95.0\scriptsize$\pm$1.2 & 89.0\scriptsize$\pm$2.2 & 83.4\scriptsize$\pm$3.1 \\
\ours{}-mp     & 97.7\scriptsize$\pm$0.3 & 97.3\scriptsize$\pm$0.3 & \textbf{95.8}\scriptsize$\pm$0.8 & 91.7\scriptsize$\pm$2.1 & 87.1\scriptsize$\pm$4.3 \\
\textbf{\ours{}} & 97.5\scriptsize$\pm$0.3 & 97.2\scriptsize$\pm$0.4 & 95.5\scriptsize$\pm$1.2 & \textbf{92.3}\scriptsize$\pm$1.6 & \textbf{88.6}\scriptsize$\pm$2.4 \\
\bottomrule
\end{tabular}
\end{table}

\Cref{tab:unseen_jpeg} evaluates the hardest setting: unseen generators under JPEG compression.
\ours{} outperforms CLIP+LoRA by $+9.1\%$ on Wukong at $Q{=}30$ ($86.9$ \vs{} $77.8$, $p{<}0.001$), demonstrating that patch-level transitions generalize well to unseen generation processes even under heavy compression.

\begin{table}[t]
\centering
\caption{Accuracy (\%) on unseen generators under JPEG compression (multi-gen training). Mean$\pm$std ($n{=}10$).}
...
\midrule
CLIP+LoRA      & 94.8\scriptsize$\pm$2.2 & 80.0\scriptsize$\pm$6.2 & 96.3\scriptsize$\pm$0.9 & 77.8\scriptsize$\pm$4.1 \\
CLS-LTD        & \textbf{96.2}\scriptsize$\pm$1.3 & 82.0\scriptsize$\pm$8.1 & \textbf{97.0}\scriptsize$\pm$0.5 & 77.4\scriptsize$\pm$3.8 \\
\ours{}-mp     & 96.6\scriptsize$\pm$1.5 & \textbf{86.6}\scriptsize$\pm$3.7 & 96.0\scriptsize$\pm$0.9 & 85.4\scriptsize$\pm$5.4 \\
\textbf{\ours{}} & 95.2\scriptsize$\pm$2.0 & 83.8\scriptsize$\pm$2.7 & 95.8\scriptsize$\pm$0.6 & \textbf{86.9}\scriptsize$\pm$2.7 \\
\bottomrule
\end{tabular}
\end{table}

\Cref{fig:jpeg_curve} visualizes the accuracy degradation curve across JPEG quality factors.
\ours{} (red) achieves the highest accuracy at low quality factors with the smallest error bars, indicating both better robustness and lower variance.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\linewidth]{../visualizations/jpeg_robustness_curve.png}
    \caption{JPEG robustness curves under multi-generator training (mean$\pm$std, $n{=}10$). \ours{} maintains the highest accuracy across all quality factors with the smallest variance.}
...
\begin{table}[t]
\centering
\caption{JPEG robustness (\%) under single-generator training (SD1.4 only). Mean$\pm$std ($n{=}10$).}
...
CLS-only transitions provide marginal benefit ($+0.5\%$), as the CLS token compresses all spatial information into one vector.
Patch-level transitions with mean pooling achieve $+4.2\%$, showing that spatial information is valuable.
The Transformer aggregator adds a further $+1.5\%$ by selectively attending to the most informative patches, while reducing variance from $4.3$ to $2.4$.

\begin{table}[t]
\centering
\caption{Component ablation (multi-gen, $Q{=}30$, $n{=}10$).}
...
\midrule
None (CLIP+LoRA)   & --            & 82.9\scriptsize$\pm$3.9 & --     \\
CLS-only           & mean          & 83.4\scriptsize$\pm$3.1 & +0.5   \\
Patch-level        & mean          & 87.1\scriptsize$\pm$4.3 & +4.2   \\
\textbf{Patch-level} & \textbf{Transformer} & \textbf{88.6}\scriptsize$\pm$\textbf{2.4} & \textbf{+5.7} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{LoRA rank sensitivity (multi-gen, $n{=}3$ for rank 4/16, $n{=}10$ for rank 8).}
\label{tab:lora_rank}
...
8 (default)& 97.5\scriptsize$\pm$0.3 & \textbf{88.6}\scriptsize$\pm$2.4 \\
...
However, Average Precision (AP), which measures ranking quality independent of threshold, reveals a stark difference: \ours{} retains AP$=$0.828$\pm$0.065 while CLIP+LoRA drops to 0.623$\pm$0.061 ($n{=}5$)---a gap of $+0.205$ in AP that is far larger than the difference between patch-level and CLS-level transitions ($+0.018$), indicating that the primary robustness gain comes from using \emph{any} transition features rather than the specific granularity.
...
\begin{table}[t]
\centering
\caption{JPEG robustness \emph{without} JPEG augmentation (multi-gen, $n{=}5$). Accuracy collapses for all methods, but AP reveals inherent feature robustness.}
\label{tab:noaug}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lcccc}
\toprule
...
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
                        & Acc (\%) & AP    & Acc (\%) & AP \\
\midrule
CLIP+LoRA      & 53.4\scriptsize$\pm$2.1 & 0.623\scriptsize$\pm$.061 & 51.9\scriptsize$\pm$1.2 & 0.556\scriptsize$\pm$.048 \\
CLS-LTD        & 55.4\scriptsize$\pm$2.0 & 0.810\scriptsize$\pm$.067 & 53.2\scriptsize$\pm$1.4 & 0.753\scriptsize$\pm$.080 \\
\ours{}-mp     & 52.7\scriptsize$\pm$1.1 & 0.818\scriptsize$\pm$.053 & 51.5\scriptsize$\pm$0.8 & 0.811\scriptsize$\pm$.058 \\
\textbf{\ours{}} & \textbf{53.8}\scriptsize$\pm$1.6 & \textbf{0.828}\scriptsize$\pm$.065 & \textbf{52.8}\scriptsize$\pm$1.7 & \textbf{0.837}\scriptsize$\pm$.066 \\
\bottomrule
\end{tabular}
\end{table}
...
As shown in \Cref{tab:dcpt} ($n{=}3$), DCPT modestly improves CLIP+LoRA at $Q{=}30$ ($82.9 \to 85.9$) but \emph{hurts} \ours{} ($88.6 \to 87.9$).
\ours{} \emph{without} DCPT ($88.6\%$) surpasses both CLIP+LoRA with DCPT ($85.9\%$) and \ours{} with DCPT ($87.9\%$), while preserving clean accuracy.
Combined with the no-augmentation results above, this confirms that patch-level transitions already capture compression-robust features, making DCPT redundant and even slightly harmful when combined with \ours{}.

\begin{table}[t]
\centering
\caption{Comparison with DCPT (multi-gen). Non-DCPT rows: $n{=}10$. DCPT rows: $n{=}3$.}
\label{tab:dcpt}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lccc}
\toprule
...
\midrule
CLIP+LoRA             & 97.8\scriptsize$\pm$0.5 & 82.9\scriptsize$\pm$3.9 & 77.8\scriptsize$\pm$4.1 \\
CLIP+LoRA + DCPT      & 96.6\scriptsize$\pm$0.4 & 85.9\scriptsize$\pm$7.1 & 84.8\scriptsize$\pm$6.5 \\
\ours{}               & 97.5\scriptsize$\pm$0.3 & \textbf{88.6}\scriptsize$\pm$2.4 & \textbf{86.9}\scriptsize$\pm$2.7 \\
\ours{} + DCPT        & 96.1\scriptsize$\pm$0.7 & 87.9\scriptsize$\pm$2.5 & 86.4\scriptsize$\pm$3.2 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Scalability to ViT-L/14}
\label{sec:vitl}

To test scalability, we evaluate on ViT-L/14 (\texttt{laion2b\_s32b\_b82k}) with layers $\{5,11,17,23\}$ (evenly spaced across 24 blocks).
As shown in \Cref{tab:vitl}, the larger backbone dramatically improves the baseline's JPEG robustness ($82.9 \to 95.6$ at $Q{=}30$), and CLIP+LoRA outperforms \ours{} on 4 of 5 metrics.
\ours{} retains an advantage on the challenging VQDM generator ($78.8$ \vs{} $75.5$), but the JPEG robustness gap disappears.
This suggests that \ours{}'s contribution is most valuable for medium-scale backbones where the CLS token's representational capacity is a bottleneck; larger backbones encode sufficient spatial information in the CLS token alone.

\begin{table}[t]
\centering
\caption{ViT-L/14 results (multi-gen, $n{=}3$).}
\label{tab:vitl}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccccc}
\toprule
Method & Clean & $Q{=}30$ & SD1.5 & GLIDE & VQDM \\
\midrule
CLIP+LoRA  & \textbf{98.7}\scriptsize$\pm$0.5 & \textbf{95.6}\scriptsize$\pm$1.8 & \textbf{93.2}\scriptsize$\pm$0.5 & \textbf{98.5}\scriptsize$\pm$0.7 & 75.5\scriptsize$\pm$1.4 \\
\ours{}    & 97.7\scriptsize$\pm$0.0 & 94.5\scriptsize$\pm$0.9 & 91.7\scriptsize$\pm$1.6 & 97.8\scriptsize$\pm$0.5 & \textbf{78.8}\scriptsize$\pm$6.6 \\
\bottomrule
\end{tabular}
\end{table}
...
Patch-level transitions capture structural inconsistencies that generalize across different generation processes, explaining the $+5.7\%$ improvement ($p{=}0.001$) in this setting.
The lower variance (std$=$2.4 \vs{} 3.9) further suggests that patch transitions provide a more stable optimization landscape.
...
Through evaluation with 10 random seeds on GenImage, we showed two main benefits of patch-level transitions under multi-generator training:
(1)~a statistically significant improvement on the training generator ($+5.7\%$ at $Q{=}30$, $p{=}0.001$, $n{=}10$) and on unseen generators ($+9.1\%$ on Wukong at $Q{=}30$, $p{<}0.001$), and
(2)~substantially lower variance across seeds (std$=$2.4 \vs{} 3.9), providing a more reliable performance floor for deployment.
A no-augmentation experiment ($n{=}5$) further demonstrates that \ours{}'s features are inherently compression-robust at the representation level (AP$=$0.828 \vs{} 0.623), independent of JPEG training augmentation.

\noindent\textbf{Limitations.}
In single-generator training, \ours{} does not improve over CLIP+LoRA.
On ViT-L/14, the larger backbone's CLS token already captures sufficient spatial information, reducing \ours{}'s marginal benefit; the method is thus most valuable for medium-scale backbones in multi-source deployment.
All experiments use GenImage; evaluation on additional benchmarks (ForenSynths, DiffusionForensics) and newer generators (SDXL, DALL-E~3, Flux) remains for future work.
