...
However, Average Precision (AP), which measures ranking quality independent of threshold, reveals a stark difference: \ours{} retains AP$=$0.828$\pm$0.065 while CLIP+LoRA drops to 0.623$\pm$0.061 ($n{=}5$).
...
As shown in \Cref{tab:dcpt} ($n{=}3$), DCPT modestly improves CLIP+LoRA at $Q{=}30$ ($84.8 \to 85.9$) but \emph{hurts} \ours{} ($89.4 \to 87.9$).
\ours{} \emph{without} DCPT ($89.4\%$) surpasses both CLIP+LoRA with DCPT ($85.9\%$) and \ours{} with DCPT ($87.9\%$), while preserving clean accuracy.
