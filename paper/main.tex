...
A no-augmentation experiment further demonstrates inherent feature-level robustness (AP$=$0.924 \vs{} 0.532 without JPEG training augmentation).
...
To verify that \ours{}'s advantage is not specific to the ViT-B/16 backbone, we evaluate on ViT-L/14 (\texttt{laion2b\_s32b\_b82k}) with layers $\{5,11,17,23\}$ (evenly spaced across 24 blocks).
As shown in \Cref{tab:vitl}, both methods benefit from the larger backbone, and \ours{} maintains its advantage on VQDM ($78.8$ \vs{} $75.5$) while achieving comparable JPEG robustness.
...
Through evaluation with 5 random seeds on GenImage, we showed two main benefits of patch-level transitions under multi-generator training:
...
A no-augmentation experiment further demonstrates that \ours{}'s features are inherently compression-robust at the representation level (AP$=$0.924 \vs{} 0.532), independent of JPEG training augmentation.

\noindent\textbf{Limitations.}
In single-generator training, \ours{} does not improve over CLIP+LoRA, suggesting the method is most beneficial for multi-source deployment scenarios.
Future work includes investigating learnable layer selection strategies as in LTD~\cite{yang2026ltd} and evaluation on additional benchmarks and newer generators.
