...
Experiments on GenImage with three random seeds show that \ours{} provides consistent improvement under JPEG compression in the multi-generator training setting ($+7.7\%$ accuracy at $Q{=}30$ over CLIP+LoRA, $90.8{\pm}1.7\%$ \vs{} $83.1{\pm}3.6\%$), with notably lower variance across seeds.
...
Our approach differs fundamentally: rather than training strategies for robustness, \ours{} provides \emph{inherently} robust features through architectural design.
...
\begin{figure}[t]
    ...
    \fbox{\parbox{0.9\linewidth}{\centering\vspace{2em}Architecture Diagram Placeholder\vspace{2em}}}
    \caption{Overview of \ours{}. Patch tokens from selected intermediate layers are differenced to compute per-patch transitions, projected and aggregated via a Transformer encoder, then fused with the CLS feature for classification.}
...
(1)~CLIP+LoRA: CLIP ViT-B/16 with LoRA and an MLP classifier (no transition analysis);
...
\begin{table}[t]
\centering
\caption{Cross-generator accuracy (\%) under multi-generator training (SD1.4+BigGAN+ADM). Mean$\pm$std over 3 seeds.}
\label{tab:cross_gen}
...
\midrule
CLIP+LoRA      & 97.9\scriptsize$\pm$0.3 & 93.7\scriptsize$\pm$0.4 & 95.9\scriptsize$\pm$0.2 & 97.1\scriptsize$\pm$1.2 & 96.4\scriptsize$\pm$0.6 & 64.4\scriptsize$\pm$4.0 \\
CLS-LTD        & \textbf{98.7}\scriptsize$\pm$0.1 & \textbf{94.0}\scriptsize$\pm$0.6 & 95.3\scriptsize$\pm$0.6 & \textbf{98.0}\scriptsize$\pm$0.1 & \textbf{97.1}\scriptsize$\pm$0.2 & 68.2\scriptsize$\pm$2.4 \\
\ours{}-mp     & 97.9\scriptsize$\pm$0.1 & 92.8\scriptsize$\pm$1.1 & 94.9\scriptsize$\pm$1.0 & 97.9\scriptsize$\pm$1.1 & 96.0\scriptsize$\pm$0.3 & \textbf{73.3}\scriptsize$\pm$1.6 \\
\textbf{\ours{}} & 97.5\scriptsize$\pm$0.2 & 93.2\scriptsize$\pm$0.6 & \textbf{96.0}\scriptsize$\pm$0.5 & 97.0\scriptsize$\pm$0.2 & 95.8\scriptsize$\pm$0.6 & 72.5\scriptsize$\pm$2.4 \\
\bottomrule
\end{tabular}
\end{table}

\Cref{tab:jpeg} presents the key result: JPEG compression robustness.
\ours{} achieves $90.8{\pm}1.7\%$ at $Q{=}30$, outperforming CLIP+LoRA by $+7.7\%$ and CLS-LTD by $+7.4\%$, with notably lower variance.
The confidence intervals between \ours{} and baselines do not overlap at $Q{=}30$, confirming statistical significance.

\begin{table}[t]
\centering
\caption{JPEG robustness (\%) under multi-generator training. Evaluated on the training generator (SD1.4) at various JPEG quality factors.}
\label{tab:jpeg}
...
\midrule
CLIP+LoRA      & 97.9\scriptsize$\pm$0.3 & 97.7\scriptsize$\pm$0.2 & 95.5\scriptsize$\pm$1.1 & 90.1\scriptsize$\pm$2.2 & 83.1\scriptsize$\pm$3.6 \\
CLS-LTD        & \textbf{98.7}\scriptsize$\pm$0.1 & \textbf{98.3}\scriptsize$\pm$0.1 & 94.4\scriptsize$\pm$0.6 & 88.0\scriptsize$\pm$1.8 & 83.4\scriptsize$\pm$3.2 \\
\ours{}-mp     & 97.9\scriptsize$\pm$0.1 & 97.6\scriptsize$\pm$0.2 & \textbf{96.7}\scriptsize$\pm$0.6 & 92.3\scriptsize$\pm$2.1 & 88.2\scriptsize$\pm$3.8 \\
\textbf{\ours{}} & 97.5\scriptsize$\pm$0.2 & 97.3\scriptsize$\pm$0.4 & 95.9\scriptsize$\pm$0.9 & \textbf{93.7}\scriptsize$\pm$1.1 & \textbf{90.8}\scriptsize$\pm$1.7 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Accuracy (\%) on unseen generators under JPEG compression (multi-gen training).}
...
\midrule
CLIP+LoRA      & 95.8\scriptsize$\pm$1.6 & 79.9\scriptsize$\pm$7.4 & 96.8\scriptsize$\pm$0.6 & 78.7\scriptsize$\pm$3.3 \\
CLS-LTD        & 95.9\scriptsize$\pm$1.1 & 82.5\scriptsize$\pm$3.2 & \textbf{97.1}\scriptsize$\pm$0.2 & 77.6\scriptsize$\pm$3.8 \\
\ours{}-mp     & \textbf{97.8}\scriptsize$\pm$0.6 & \textbf{87.4}\scriptsize$\pm$2.6 & 95.9\scriptsize$\pm$0.8 & 84.8\scriptsize$\pm$5.4 \\
\textbf{\ours{}} & 96.4\scriptsize$\pm$0.5 & 85.4\scriptsize$\pm$1.7 & 96.0\scriptsize$\pm$0.4 & \textbf{89.6}\scriptsize$\pm$1.7 \\
\bottomrule
\end{tabular}
\end{table}
...
\ours{} achieves $88.9{\pm}2.1\%$ \vs{} $88.0{\pm}1.5\%$ for CLIP+LoRA at $Q{=}30$---a difference of only $+0.9\%$ with overlapping confidence intervals, which is not statistically significant.
This contrasts with the multi-generator setting where the gap is $+7.7\%$.
The advantage of patch-level transitions emerges specifically when the model must generalize across diverse generation processes, suggesting that spatial transition patterns encode generator-agnostic forensic information.

\begin{table}[t]
\centering
\caption{JPEG robustness (\%) under single-generator training (SD1.4 only).}
...
Method & Clean & $Q{=}95$ & $Q{=}75$ & $Q{=}50$ & $Q{=}30$ \\
\midrule
CLIP+LoRA      & 98.4\scriptsize$\pm$0.2 & 98.0\scriptsize$\pm$0.1 & 95.1\scriptsize$\pm$1.1 & 91.1\scriptsize$\pm$1.0 & 88.0\scriptsize$\pm$1.5 \\
CLS-LTD        & \textbf{98.9}\scriptsize$\pm$0.0 & \textbf{98.2}\scriptsize$\pm$0.1 & 92.2\scriptsize$\pm$1.6 & 83.8\scriptsize$\pm$3.0 & 81.3\scriptsize$\pm$1.6 \\
\ours{}-mp     & 98.0\scriptsize$\pm$0.4 & 97.8\scriptsize$\pm$0.2 & 94.5\scriptsize$\pm$0.8 & 89.8\scriptsize$\pm$1.2 & 86.2\scriptsize$\pm$1.6 \\
\textbf{\ours{}} & 98.7\scriptsize$\pm$0.9 & 98.3\scriptsize$\pm$1.1 & \textbf{95.8}\scriptsize$\pm$0.9 & \textbf{91.7}\scriptsize$\pm$1.7 & \textbf{88.9}\scriptsize$\pm$2.1 \\
\bottomrule
\end{tabular}
\end{table}
...
The Transformer aggregator adds a further $+2.6\%$ by selectively attending to the most informative patches, while also reducing variance from $3.8$ to $1.7$.

\begin{table}[t]
\centering
\caption{Component ablation (multi-gen, $Q{=}30$, mean$\pm$std over 3 seeds).}
...
\midrule
None (CLIP+LoRA)   & --            & 83.1\scriptsize$\pm$3.6 & --     \\
CLS-only           & mean          & 83.4\scriptsize$\pm$3.2 & +0.3   \\
Patch-level        & mean          & 88.2\scriptsize$\pm$3.8 & +5.1   \\
\textbf{Patch-level} & \textbf{Transformer} & \textbf{90.8}\scriptsize$\pm$\textbf{1.7} & \textbf{+7.7} \\
\bottomrule
\end{tabular}
\end{table}
...
We examine whether \ours{} is complementary to degradation-consistent paired training (DCPT)~\cite{dcpt2026}.
As shown in \Cref{tab:dcpt}, DCPT substantially improves CLIP+LoRA ($+4.8\%$ at $Q{=}30$) but provides no benefit for \ours{} ($-0.6\%$).
Notably, \ours{} \emph{without} DCPT ($90.8\%$) already surpasses CLIP+LoRA \emph{with} DCPT ($87.9\%$), while preserving clean accuracy.
This indicates that patch-level transitions provide inherent compression robustness through architecture design, making augmentation-based strategies redundant.

\begin{table}[t]
\centering
\caption{Comparison with DCPT (multi-gen, single run).}
\label{tab:dcpt}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lccc}
\toprule
...
\midrule
CLIP+LoRA             & 97.9 & 83.1 & 78.7 \\
CLIP+LoRA + DCPT      & 96.1 & 87.9 & 87.5 \\
\ours{}               & 97.5 & \textbf{90.8} & \textbf{89.6} \\
