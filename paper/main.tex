...
Experiments on GenImage with five random seeds show that \ours{} improves JPEG compression robustness in the multi-generator training setting ($89.4{\pm}2.6\%$ \vs{} $84.8{\pm}3.9\%$ at $Q{=}30$), with significantly lower variance and a statistically significant advantage on unseen generators ($p{=}0.010$).
Ablation confirms that patch-level transitions outperform CLS-only transitions, and a no-augmentation experiment demonstrates inherent feature-level robustness (AP$=$0.924 \vs{} 0.532 without JPEG training augmentation).
...
For instance, a CLIP+LoRA detector trained on clean images drops from 98.0\% to 84.8\% under JPEG quality factor $Q{=}30$ in multi-generator evaluation.
...
The improvement is statistically significant on unseen generators (\Cref{tab:unseen_jpeg}): Wukong $Q{=}30$ yields $p{=}0.010$.
...
\begin{table}[t]
\centering
\caption{Comparison with DCPT (multi-gen). Non-DCPT rows: $n{=}5$. DCPT rows: $n{=}3$ (seeds 456, 789, 1024).}
\label{tab:dcpt}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lccc}
\toprule
...
(1)~a statistically significant improvement on unseen compressed generators ($+8.4\%$ on Wukong at $Q{=}30$, $p{=}0.010$), and
(2)~substantially lower variance across seeds (std$=$2.6 \vs{} 3.9), providing a more reliable performance floor for deployment.
The mean improvement on the training generator ($+4.6\%$ at $Q{=}30$, $p{=}0.088$) shows a consistent trend but does not reach $p{<}0.05$ significance.
...
The improvement on the training generator under JPEG compression, while consistent in direction, does not reach statistical significance at $\alpha{=}0.05$ with $n{=}5$.
Future work includes investigating learnable layer selection strategies as in LTD~\cite{yang2026ltd}, increasing the number of evaluation seeds, and scaling to larger ViT backbones.
